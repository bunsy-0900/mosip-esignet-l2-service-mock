...
const sendFrame = (ws, destination, body) => {
    if (ws.readyState === WebSocket.OPEN) {
        ...
        ws.send(createStompFrame('MESSAGE', { destination, subscription: subscribeId, 'content-type': 'application/json', 'content-length': body.length }, body));
    }
}
...
const processFrame = (messageObj) => {
    ...
    if (body.stepCode === 'START') {
        const response = useCase.find(_ => (_.frameNumber === 0) && (_.stepCode === body.stepCode))
        if (response) {
            sendFrame(connectedClients[slotId], `/topic/${slotId}`, JSON.stringify(response));
        }
    }
    ...
    if (body.frames !== null && body.frames.length > 0) {
        ...
        body.frames.forEach(frame => {
            const response = useCase.find(_ => (_.frameNumber === frame.order) && (_.stepCode === body.stepCode))
            if (response) {
                sendFrame(connectedClients[slotId], `/topic/${slotId}`, JSON.stringify(response));
            }
        });
...
const onSocketConnect = (ws, req) => {
    ...
    ws.on('message', (message) => {
        ...
        if (messageObj.command === 'CONNECT') {
            ...
            ws.send(createStompFrame('CONNECTED', { version: '1.2', 'heart-beat': "0,0", 'user-name': slotId })); // Send CONNECTED frame with session ID
        } else if (messageObj.command === 'SUBSCRIBE') {
            // Handle subscription logic (e.g., store subscription topic)
            console.log(`Client ${clientId} subscribed to topic: ${messageObj.headers.destination}`);
